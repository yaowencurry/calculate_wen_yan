...
Page({
  ...
  handleHistoryData(historyId, shelfId, barId) {
    getStorage('historyData')
      .then(res => {
        const historyList = res.filter(item => item.id == historyId)[0].data;
        const shelfData = historyList.filter(item => item.shelfId == shelfId)[0];
        console.log(shelfData)
        const barData = shelfData.barList.filter(item => item.barId == barId)[0];
        console.log(barData)
        this.setData({
          list: barData.detail,
          shelfNumber: shelfData.number
        })
      })
  },
  /**
  ...
  onLoad: function (options) {
    ...
    if (!barId) return
    if (isHistory) {
      this.handleHistoryData(historyId, shelfId, barId)
    } else {
      wx.getStorage({
        key: 'shelfList',
        success: (res) => {
  ...
  onUnload: function () {
    ...
    //   title: 'cancelColor',
    // })
    if (!this.data.isHistory && !this.data.isSave && this.data.list.length > 0) {
      this.backBarList();
    }
  },
